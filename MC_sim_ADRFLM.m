...
%       Effect Models
%
% Main Matlab codes for Monte Carlo simulations on rejection probabilities with a single covariate.
%
% *****************************************************
% Subroutines directly used in this ma in code:
%    (1) CaliEstGeneral
%    (2) get_weight
%    (3) CV_K1K2General.m
%    (4) Hfunction.m
...
%% step 2.2: Select K and estimate pi_0
    ...
    [K1(j),K2(j)] = CV_K1K2General(umat,vmat,Ti,Y,p);
    
    weight = get_weight(umat,vmat,K1(j),K2(j));
    
%% step 2.3: Estiamte the parameters and Residual U
    
    [gEst,gGrad,thetahat(:,j)]= CaliEstGeneral(Ti,Ti,weight,Y,p);
...
%% step 2.4: Test the model (calculate the p-value)
    ...
    [JhatN,JNstar] = JNJNstarADRFLM(N,B,p,H,X,Ti,Y,weight,U(:,j),gGrad,gEst);
...
filename=sprintf('DGP%s-N%d-%s.mat',DGP,N,date);
save(filename)
